;(function () {
  var $win = $(window),
	...
	lpc_template.queue.lpcWrapText = function ($self) {
	    var $blocks = $self.hasClass('js-wrap-text') ? $self : $self.find('.js-wrap-text');
	    
	    if (!$blocks.length) return;
	
	    $blocks.each(function () {
	        var $block = $(this);
	        var bigTextEl = $block.find('.lpc-text-10-big-text-body')[0];
	        var wrapEl = $block[0];
	
	        // 1. Функция синхронизации fontSize для конкретного блока
	        function syncFontSize() {
	            if (bigTextEl && wrapEl) {
	                const style = window.getComputedStyle(bigTextEl);
	                wrapEl.style.fontSize = style.fontSize;
	            }
	        }
	
	        syncFontSize();
	
	        // Наблюдатель за изменениями стиля конкретного заголовка
	        if (bigTextEl) {
	            const observer = new MutationObserver(syncFontSize);
	            observer.observe(bigTextEl, {
	                attributes: true,
	                attributeFilter: ['style', 'class']
	            });
	        }
	
	        // 2. Логика обтекания (только если есть внутренний блок)
	        var $inner = $block.find('.lpc-text-10-inner._bottom-vertical');
	        
	        if ($inner.length) {
	            $(window).on("resize.lpcWrap", function () {
	                setTimeout(function () {
	                    var mediaTypeCheck = $('.decor-wrap').attr('data-media-source');
	                    
	                    if (mediaTypeCheck === "media-xs" || mediaTypeCheck === "media-sm") {
	                        $inner.removeAttr('style');
	                    } else {
	                        const $bigText = $block.find('.lpc-text-10-big-text-body');
	                        const $smallText = $block.find('.lpc-text-10-small-text-body');
	                        
	                        // Получаем line-height
	                        let lineHeight = parseFloat($bigText.css('line-height'));
	                        if (isNaN(lineHeight) || lineHeight < 10) {
	                            lineHeight = parseFloat($bigText.css('font-size'));
	                        }
	
	                        const totalHeight = $bigText.height();
	                        const totalSmallHeight = $smallText.height();
	                        
	                        // Расчеты
	                        const offset = lineHeight;
	                        const propValue = Math.ceil(totalSmallHeight / lineHeight) * offset;
	                        const smallGap = totalHeight - propValue + 1;
	
	                        $inner.css({
	                            'padding-top': smallGap + 'px',
	                            'shape-outside': `inset(${smallGap}px 0 0 0)`,
	                            'height': propValue + 'px'
	                        });
	                        
	                        /* Консоль для отладки конкретного блока */
	                        // console.log('Block processed:', wrapEl);
	                    }
	                }, 200);
	            }).trigger("resize.lpcWrap");
	        }
	    });
	};
  ...
  lpc_template.initGoogleMaps = function (options) {
    ...
    $.each(options.data, function (key, item) {
	  ...
	  //https://pastebin.com/zK79krwA старая версия
	lpc_template.initYandexMaps = function (options, objectListFlag) {
		...
		var MyBalloonContentLayoutClass = ymaps.templateLayoutFactory.createClass(
	        options.customballon || 
	        '<div class="baloon-content" style="padding: 0 10px;">' +
	        '<h5 style="margin: 0;">$[properties.name]</h5>' +
	        '<p>$[properties.desc]</p></div>'
	    );
	
	    // 2. Макет кастомной метки (используется только если options.customPin == true)
	    var MyIconContentLayout = ymaps.templateLayoutFactory.createClass(
	        '<div class="map-label-wrapper">' +
	            '<div class="map-label-img" style="background-image: url($[properties.image]);"></div>' +
	            '<span class="map-label-name">$[properties.name]</span>' +
	        '</div>'
	    );
	    ...
	    if (options.cluster) {
			myCollection = new ymaps.Clusterer({
			    ...
			    clusterIconColor: options.customcolor || '#ff0000',
				groupByCoordinates: false,
				clusterDisableClickZoom: false,
				clusterHideIconOnBallonOpen: false,
				geoObjectHideIconOnBallonOpen: false
			});
		} else {
  ...
  lpc_template.queue.formInputs = function ($self) {
	...
	if($block.length) {
		...
		$block.each(function () {
			...
			function sliderInit(parent){
				...
				//Счетчики 
				let counterIndex = sliderBlock.querySelector('.magic-slider__index');
				let totalItems = slideItem.length;
				//Счетчики 
				...
				function sliderInitInter() {	
						...
						} else if (sliderBlock.classList.contains('magic-slider-small')){
						
						    if (decorWrap.getAttribute('data-media-source') === 'media-xs'){
						        itemWidth = slideWidth;
						
						    } else if(decorWrap.getAttribute('data-media-source') === 'media-sm'){
						        itemWidth = slideWidth / 4 * 3;
						    coefficient = itemWidth / 3        
						    } else{
						        itemWidth = slideWidth / 2;
						        coefficient = itemWidth
						    }
				...
				function initSlider() {
					...
					if(scrollWidth > 0) {
					    ...
					    .on("progress", function (event) {
						    ...
						    let startZone = 0.1; // 10% в начале (ничего не движется)
						    let endZone = 0.1;   // 10% в конце (ничего не движется)
						    
						    let adjustedProgress = 0;
						
						    if (progress < startZone) {
						        adjustedProgress = 0;
						    } else if (progress > (1 - endZone)) {
						        adjustedProgress = 1;
						    } else {
						        adjustedProgress = (progress - startZone) / (1 - startZone - endZone);
						    }
						
						    let transformPos = Math.round(-scrollWidth * adjustedProgress);
						    
						    slider.style.transform = "translateX(" + transformPos + "px)";
						    
						    // Обновление счетчика страниц на основе скорректированного прогресса
						    if (counterIndex) {
						        let currentIndex = Math.round(adjustedProgress * (totalItems - 1)) + 1;
						        if (counterIndex.textContent != currentIndex) {
						            counterIndex.textContent = currentIndex;
						        }
						    }
						})
					    .addTo(controller);
				...
				function handleAttributeChange() {
				    ...
				    if(sliderBlock.classList.contains('lpc_scroll_disable')) {
					    ...
					    if (mediaSource === 'media-xs' || mediaSource === 'media-md' || mediaSource === 'media-sm') {
					        ...
					        if(sliderBlock.classList.contains('magic-slider-full-width')) {//Для адаптации , когда блок на всю ширину
						        let slideWidth = slideWrap.offsetWidth;
								let docWidth = document.documentElement.clientWidth;
								let itemMarginLeft = slideWrap.getBoundingClientRect().left;
								let itemMarginRight = docWidth - slideWrap.getBoundingClientRect().right;
	...
	lpc_template.queue.accordeon = function ($self) {
	    ...
	    $block.each(function () {
	        ...
	        $accordionItemWrap.click(function(event){
        		if (!$(event.target).closest($accordionHeader).length && !$(event.target).closest('.lpc-accordion-content').length){
	        ...
	        $accordionHeader.click(function () {
	            ...
	            let $accordionToggle = $this.closest('.lpc-accordion-toggle');
	            
	            if($accordionToggle && !parent.hasClass('active')) {			
					$accordionToggle.find('.lpc-accordion-item.active .lpc-accordion-content').hide();
					$accordionToggle.find('.lpc-accordion-item.active').removeClass('active');					
				}
	...
	lpc_template.queue.lpcTamlim = function ($self) {
	    var $block = $self.find(".lpc-tamlim-init");
	    ...
	    $block.each(function () {
	        ...
	        items.each(function (i) {
	            ...
	            if (next && next.classList.contains('_flag_title')) {
	                current.classList.add('_before_flag');
	            }
	        });
	    });
	};
	...
	lpc_template.queue.furniturePoints = function ($self) {

		let $blocks = $self.find('.furniture-points-init');
		...
		$blocks.each(function () {
			...
			const $block  = $(this);
			const $items  = $block.find('.lpc-furniture-points__point');
			const $image  = $block.find('.lpc-furniture-points__image-box');
			...
			const ns = '.furniturePoints_' + Math.random().toString(36).slice(2);
			...
			$items.each(function (i) {
				$(this).on('click', function (e) {
					...
					if ($(e.target).closest('.lpc-furniture-points__point-number').length) return;
					if ($(e.target).closest('.lpc-furniture-points__tooltip').length) return;
			...
			$items.on('mousedown', function (e) {
				...
				isDragging = true;
				$currentItem = $(this).closest('.lpc-furniture-points__point');
	
				e.preventDefault();
				e.stopPropagation();
			});
	
			$(document).off('mousemove' + ns).on('mousemove' + ns, function (e) {
	
				if (!isDragging || !$currentItem || !$image.length) return;
	
				const rect = $image.get(0).getBoundingClientRect();
				...
				updateCoords($currentItem, xVal, yVal);
			});
	
			$(document).off('mouseup' + ns).on('mouseup' + ns, function () {
			...
			function updateCoords($item, x, y) {
				const $inputX = $item.find('input[name="point_x"]');
				const $inputY = $item.find('input[name="point_y"]');
	
				if ($inputX.length) {
					$inputX.val(x).trigger('change');
				}
	
				if ($inputY.length) {
					$inputY.val(y).trigger('change');
				}
	
				const $coords = $item.find('.lpc-furniture-points__point-coords');
	...
	lpc_template.queue.memorialParts = function ($self) {

		let $blocks = $self.find('.memorial-parts-init');
		...
		$blocks.each(function () {
			...
			const $block  = $(this);
			const $points = $block.find('.lpc-memorial-parts__point');
			const $items  = $block.find('.lpc-memorial-parts__item');
			const $image  = $block.find('.lpc-memorial-parts__visual-box');
			...
			const ns = '.memorialParts_' + Math.random().toString(36).slice(2);
			...
			function setActive(index){
				...
				const $point = $points.eq(index);
				const $item  = $items.eq(index);
	
				$point.addClass('_active');
				$item.addClass('_active');
			}
			...
			$points.on('mousedown', function(e){
			...
			$(document).off('mousemove'+ns).on('mousemove'+ns,function(e){
				...
				updateCoords($currentPoint, xVal, yVal);
			...
			function updateCoords($point, x, y){
				
				const $inputX = $point.find('input[name="point_x"]');
				const $inputY = $point.find('input[name="point_y"]');
				...
				const $coords = $point.find('.lpc-memorial-parts__point-coords');
	...
	lpc_template.queue.furniturePoints = function ($self) {
		...
		$blocks.each(function () {
			...
			// ДВЕ ГРУППЫ ТОЧЕК
			const $pointsImage = $block.find('.lpc-furniture-points__points._image_points .lpc-furniture-points__point');
			const $pointsList  = $block.find('.lpc-furniture-points__points:not(._image_points) .lpc-furniture-points__point');
	
			const $numbers = $block.find('.lpc-furniture-points__point-number');
			const $cards   = $block.find('.lpc-furniture-points__tooltip');
			const $image   = $block.find('.lpc-furniture-points__image-box');
	
			if (!$pointsImage.length && !$pointsList.length) return;
			...
			function getConfiguredMode() {
				return $block.data('interaction') === 'hover' ? 'hover' : 'click';
			}
			
			function getCurrentMedia() {
				const $decorWrap = document.querySelector('.decor-wrap');
				return $decorWrap ? $decorWrap.dataset.mediaSource : '';
			}
	
			function getInteractionMode() {
				const media = getCurrentMedia();
	
				if (media === 'media-md' || media === 'media-sm' || media === 'media-xs') {
					return 'click';
				}
			...
			function isMobileMedia() {
				const media = getCurrentMedia();
				return media === 'media-md' || media === 'media-sm' || media === 'media-xs';
			}
	
			function setActiveById(id) {
				$pointsImage.removeClass('_active');
				$pointsList.removeClass('_active');
				$numbers.removeClass('_active');
				$cards.removeClass('_active');
	
				$pointsImage.filter(`[data-id="${id}"]`).addClass('_active');
				$pointsList.filter(`[data-id="${id}"]`).addClass('_active');
				$cards.filter(`[data-id="${id}"]`).addClass('_active');
	
				$numbers.each(function () {
					const $point = $(this).closest('.lpc-furniture-points__point');
					if ($point.data('id') == id) {
			...
			function scrollToTooltipById(id) {
				if (!isMobileMedia()) return;
	
				const $tooltip = $cards.filter(`[data-id="${id}"]`);
				if (!$tooltip.length) return;
				
				const $scrollContainer = $block.find('.lpc-furniture-points__points');
				...
				const targetLeft =
					tooltip.offsetLeft - (container.clientWidth / 2) + (tooltip.clientWidth / 2);
			...
			function activatePoint(id, withScroll = true) {
				setActiveById(id);
	
				if (withScroll) {
					scrollToTooltipById(id);
				}
			}
	

			$block.on('click', '.lpc-furniture-points__point', function (e) {
				if ($(e.target).closest('.lpc-furniture-points__tooltip').length) return;
				if ($(e.target).closest('.lpc-furniture-points__point-number').length) return;
				...
				activatePoint(id, true);
			});
	
			
			$block.on('click', '.lpc-furniture-points__point-number', function (e) {
				if (wasDragging) {
					wasDragging = false;
					return;
				}
				...
				const id = $(this).closest('.lpc-furniture-points__point').data('id');
				activatePoint(id, true);
			});
	
			
			$block.on('click', '.lpc-furniture-points__tooltip', function (e) {
				e.preventDefault();
				e.stopPropagation();
			...
			$block.on('mouseenter', '.lpc-furniture-points__point, .lpc-furniture-points__tooltip', function () {
				const mode = getInteractionMode();
				if (mode !== 'hover') return;
	
				const id = $(this).data('id');
				activatePoint(id, false);
			});
	
			$block.on('mouseenter', '.lpc-furniture-points__point-number', function () {
				const mode = getInteractionMode();
				if (mode !== 'hover') return;
			...
			$block.on('mousedown', '.lpc-furniture-points__point-number', function (e) {
				if (!s3LP.is_cms) return;
			...
			$(document).off('mousemove' + ns).on('mousemove' + ns, function (e) {
				...
				$pointsImage.filter(`[data-id="${id}"]`).css({
					left: xVal + '%',
					top: yVal + '%'
				});
	
				$pointsList.filter(`[data-id="${id}"]`).css({
					left: xVal + '%',
					top: yVal + '%'
				});
	
				updateCoords($pointsList.filter(`[data-id="${id}"]`), xVal, yVal);
			});
			...
			const firstId = $pointsList.first().data('id');
			if (firstId !== undefined) {
				activatePoint(firstId, false);
			}
	
			function updateTooltipPosition($tooltip) {
				if (!$tooltip.length) return;
	
				const $container = $block.find('.lpc-furniture-points__image-box');
				const wasHidden = !$tooltip.hasClass('_active');
			...
			document.addEventListener('dataMediaSourceChange', function () {
				const activeId = $cards.filter('._active').data('id') || $pointsList.first().data('id');
				if (activeId !== undefined) {
					activatePoint(activeId, false);
				}
			});
		});
	};
	...
	lpc_template.queue.tabsConditions = function ($self) {
	    let $block = $self.find('.conditions-tabs-init');
	
	    if (!$block.length) return;
	
	    $block.each(function () {
	        let $this = $(this);
	        let $buttons = $this.find('._tabs-button');
	        let $items = $this.find('._tabs-item');
	        let $lpButton = $this.find('.lp-button');
	        let $head = $this.find('.lpc-conditions__head');
	        
	        if ($head.hasClass('_type-group')) {
                let borderRadiusValue = $lpButton.css('border-radius');
                $head.css('border-radius', borderRadiusValue);
            }
	        ...
	        $buttons.on('click', function () {
	            ...
	            $items.each(function () {
	                let $item = $(this);
	                $item.toggleClass('active', $item.data('index') == index);
	            });
	        });
	    });
	};
	...
	lpc_template.queue.variableSlider = function ($self) {
		...
		let $block = $self.attr('data-variable-slider-init') ? $self : $self.find('[data-variable-slider-init]');
	
		if ($block.length) {
			if ($block.data('data-variable-slider-init') != true) {
	...
	lpc_template.queue.videoHoverPlay = function ($self) {
		...
		$blocks.on('mouseenter.videoHover', function () {
		    ...
		    // Очищаем таймер ухода именно для ЭТОГО блока
		    ...
		    // Сохраняем таймер наведения в этот блок
		    $this.data('hoverTimeout', hTimeout);
		});
		
		$blocks.on('mouseleave.videoHover', function () {
		    const $this = $(this);
		    
		    // Очищаем таймер наведения именно для ЭТОГО блока
		    ...
		    // Сохраняем таймер ухода в этот блок
		    $this.data('leaveTimeout', lTimeout);
		});
	};
	...
	lpc_template.queue.videoPlayButtonDefolt = function($self) {
    // 1. Предварительная настройка всех видео для Android
    $self.find('.lp-video-block-wrappper video').each(function() {
        this.setAttribute('playsinline', '');
        this.setAttribute('webkit-playsinline', '');
        this.preload = "metadata"; // Важно для Android, чтобы плеер был готов
    });

    // 2. Единый обработчик клика
    $self.on('click', '.js-lp-play-video', function(e) {
        e.preventDefault();
        e.stopPropagation();

        var $btn = $(this);
        var $wrapper = $btn.closest('.lp-video-block-wrappper');
        var video = $wrapper.find('video')[0];

        if (!video) return;

        // Сначала запускаем видео, потом меняем классы (Android любит такой порядок)
        video.controls = true;
        ...
        if (promise !== undefined) {
            promise.then(function() {
                ...
                console.log("Android Autoplay Blocked: ", error);
                // Если заблокировано, пробуем запустить с выключенным звуком (последний шанс)
                video.muted = true;
                video.play();
                $btn.addClass('hide');
            });
        } else {
            // Для совсем старых браузеров без Promise
            $btn.addClass('hide');
        }
    });

    // 3. Синхронизация состояния (если пользователь нажал на само видео)
    $self.find('.lp-video-block-wrappper video').on('play', function() {
        $(this).closest('.lp-video-block-wrappper').find('.js-lp-play-video').addClass('hide');
    }).on('pause ended', function() {
        $(this).closest('.lp-video-block-wrappper').find('.js-lp-play-video').removeClass('hide');
    });
};
  ...
  window.lp_init = function ($block) {
    ...
    $win
      .on("resize", function () {
        ...
        $(".lpc-block:not(.custom-lpc-block)").css("max-width", decorWidth);
      })
      .trigger("resize");
...
class Calculator {
    ...
    initializeRangeFields(fields) {
        fields.forEach((field) => {
            ...
            //field.addEventListener("input", updateCurrentPosition); //Старый вариант
            // 1. Событие 'change' — сработает, когда пользователь отпустил ползунок
			field.addEventListener("change", updateCurrentPosition);
			
			// 2. Дополнительно для мобильных устройств (touch-событие)
			field.addEventListener("touchend", updateCurrentPosition);
            updateCurrentPosition();
        });
    }
